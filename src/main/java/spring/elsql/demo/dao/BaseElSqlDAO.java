...
import spring.dao.demo.domain.BaseIdDomain;
...
public abstract class BaseElSqlDAO {
	...
	public BaseElSqlDAO(DataSource datasource) {
		this(datasource, ElSqlConfig.MYSQL);
	}
	...
	/**
	 * Find the matching fragment name without specific parameters
	 * in elsql if present
	 * 
	...
	/**
	 * Find the matching fragment name with given specific parameters
	 * to substitute in elsql bundle if present
	 * 
	 * @param fragmentName a fragment name to search in elsql bundle
	 ...
	 */
	protected <T extends BaseIdDomain> Optional<T> findById(String fragmentName, SqlParameterSource paramSource, RowMapper<T> rowMapper) {
		try {
			return Optional.of(getJdbcTemplate().queryForObject(getSqlString(fragmentName, paramSource), paramSource, rowMapper));
